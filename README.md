...
function gerar(){
    ...
    let html = `<table>
        <tr><th>n</th><th>log</th></tr>`;

    for(let i=s;i<=e;i++){
        html += `
        <tr>
            <td>${i}</td>
            <td>${(Math.log(i)/Math.log(b)).toFixed(p)}</td>
        </tr>`;
    }
    html += `</table>`;
...
function baixar(){
    ...
    const rows = [...preview.querySelectorAll("tr")]
        .slice(1)
        .map(r=>({
            n:r.cells[0].innerText,
            log:r.cells[1].innerText
        }));
...
function criarUsuario(){
    banco[novoUser.value] = {senha:novoPass.value};
    localStorage.setItem("usuarios",JSON.stringify(banco));
    atualizarUsuarios();
}
